...
void yuv2rgb15(int *blk,unsigned short *image) {
	...
	int Cb,Cr,R,G,B;
	int A = (mdec.command & 0x02000000) ? 0x8000 : 0;
	int *Cbblk = blk;
	...
	for (y=0; y<16; y+=2, Crblk+=4, Cbblk+=4, Yblk+=8, image+=24) {
		...
		for (x=0; x<4; x++, image+=2, Crblk++, Cbblk++, Yblk+=2) {
			...
			RGB15( 8, Yblk[DCTSIZE2+0], A);
			RGB15( 9, Yblk[DCTSIZE2+1], A);
			RGB15(24, Yblk[DCTSIZE2+8], A);
			RGB15(25, Yblk[DCTSIZE2+9], A);
		}
	} else
	for (y=0; y<16; y+=2, Yblk+=8, image+=24) {
		...
		for (x=0; x<4; x++, image+=2, Yblk+=2) {
			...
			RGB15BW( 8, Yblk[DCTSIZE2+0], A);
			RGB15BW( 9, Yblk[DCTSIZE2+1], A);
			RGB15BW(24, Yblk[DCTSIZE2+8], A);
			RGB15BW(25, Yblk[DCTSIZE2+9], A);
		}
	}
}
